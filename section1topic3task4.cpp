...
void push() {//добавление элемента в список
    if (!isFull()) {
        string _value;
        if (!isEmpty()) {//если в списке есть элементы помимо заголовчного, спросить как добавить элемент
        ...
        else {//в случае если список пуст, добавлем элемент в начало
            cout << "\nEnter yor element";
...
void find() {
    if (!isEmpty()) {
        ...
        if (current != 0) cout << "Element " << _value << "found. Under the number " << i << "(in memory cell #" << (current++) << ")\n";
        else cout << "Element " << _value << "is not found\n";
    }
